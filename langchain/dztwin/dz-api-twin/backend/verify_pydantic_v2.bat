@echo off
REM Verification script for Pydantic V2 migration (Windows)
...
REM Check Pydantic version
echo 1. Checking Pydantic version...
python -c "from pydantic import __version__; print(f'   Pydantic version: {__version__}')"
echo.

REM Test imports without warnings
echo 2. Testing imports for deprecation warnings...
python -W all -c "from main import app; from config import get_config" 2>&1 | findstr /C:"PydanticDeprecatedSince20" /C:"main.py" /C:"config.py" >nul && echo    X Found deprecation warnings || echo    + No deprecation warnings in our code
echo.

REM Run validator tests
echo 3. Running validator tests...
python test_pydantic_v2.py 2>&1 | findstr /R "^[+X]"
echo.
